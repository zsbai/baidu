...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_uikit__ = __webpack_require__(33);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_uikit___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_uikit__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_uikit_dist_js_uikit_icons__ = __webpack_require__(77);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_velocity_animate__ = __webpack_require__(78);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_css_main_less__ = __webpack_require__(79);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_css_main_less___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3_css_main_less__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__src_common_js_util__ = __webpack_require__(80);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__src_common_js_util___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_4__src_common_js_util__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__src_common_js_login__ = __webpack_require__(81);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__src_common_js_login___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_5__src_common_js_login__);
...
__WEBPACK_IMPORTED_MODULE_4__src_common_js_util___default.a.init()
// loads the Icon plugin
__WEBPACK_IMPORTED_MODULE_0_uikit___default.a.use(__WEBPACK_IMPORTED_MODULE_1_uikit_dist_js_uikit_icons___default.a)

__WEBPACK_IMPORTED_MODULE_5__src_common_js_login___default.a.init()
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(19), __webpack_require__(19)))
...
exports.default = {
  initNav: function initNav() {
    ...
    $('a[link]').map(function (i, el) {
      var elm = $(el);
      var parent = elm.parent();
      var link = elm.attr('link');
      var href = elm.attr('href');
      var subHref = href.split('/').pop();
      var matched = link.length ? link.split(',').map(function (i) {
        return $.trim(i).split('/').pop();
      }).indexOf(path) > -1 : subHref === path;

      matched ? parent.addClass('uk-active') : parent.removeClass('uk-active');
    });
  },
  init: function init() {
    this.initNav();
    $(window).bind('hashchange', this.initNav);

    // $('.header-menu-nav').mouseenter(()=>$(this).addClass('hover-active'))
    // $('.header-menu-nav').mouseout(()=>$(this).('uk-active'))
    $('a[link]').map(function (i, el) {
      var elm = $(el);
      elm.click(function () {
...
var iframeOnLoad = function () {
  var _ref = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee() {
    var search, callbackParams, u, castk, $jumpIframe;
    return _regenerator2.default.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            search = $(this).prop('contentWindow').location.search;
            callbackParams = (0, _qs.parse)(search.substring(1));

            showLoading();
            // clean() // 新登录不用
            if (callbackParams.errno) {
              hideLoading();
              // $('#imagecode').click()
              // switch (callbackParams.errno) {
              //   case '131':
              //     $('#imagecode-error')
              //       .text('验证码错误')
              //       .show()
              ...
              //   case '135':
              //     $('#password-error')
              //       .text('多次密码输入错误，账户被锁定，请使用“找回密码”功能解锁')
              //       .show()
              //     break
              //   default:
              //     $('#name-error')
              //       .text(callbackParams.e)
              //       .show()
              // }
            } else if (callbackParams.redirecturl) {
              // let { u } = parse(callbackParams.redirecturl)
              u = callbackParams.redirecturl;
              castk = u && u.split('?') && u.split('?')[1];
              $jumpIframe = $('#uc-jump-iframe');
              // $jumpIframe.prop('contentWindow').location.href = BASE_URL + u
              // $jumpIframe.prop('contentWindow').location.href = u
              // 直接跳到union.baidu.com 不走iframe以及iframe的href变化触发的fetchUserInfo，fetchUserInfo函数的逻辑也有一些问题
  ...
  return function iframeOnLoad() {
    return _ref.apply(this, arguments);
  };
}();

var _qs = __webpack_require__(101);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

// import { Component, Vue, Prop } from 'vue-property-decorator'
// import { Environment, logon } from '@/apis/login'
var SELF_URL = window.location.host;
var IMG_CODE_URL = '//cas.baidu.com/?action=image2&appid=6&key=';
var BASE_URL = 'http://union.baidu.com?';
// const TEST_URL = 'http://unionqa.baidu.com?'
var MANAMGER_URL_PREFIX = '/admin/';
var BAIQINGTENG = 'http://mssp.baidu.com/baiqingteng';
function imageCodeHandler() {
  $('#imagecode').click(function () {
    return $('#imagecode img').attr('src', '' + IMG_CODE_URL + Date.now());
  });
}
function initIframe() {
  $('#new-login-iframe').on('load', iframeOnLoad);
  $('#uc-jump-iframe').on('load', fetchUserInfo);
}

function initBtn() {
  $('#access-btn').click(function () {
    // if(window.experince === true) {
    //   window.location.href = BAIQINGTENG
    // }
    // else {
...
function initLoginPanel() {
  $('#header-secondary-login').click(function () {
    $('.header-secondary-login-panel').toggleClass('show');
  });
}

function fetchUserInfo() {
  // 判断管理员账户登录状态
  // $.ajax('/userlogin.action').done((result, text, xhr) => {
  //   const header = xhr.getAllResponseHeaders()
  //   debugger
  // })
  var jumpUrl = $('#uc-jump-iframe').prop('contentWindow').location;
  if (jumpUrl.pathname.indexOf(MANAMGER_URL_PREFIX) === 0 || jumpUrl.hostname.indexOf('mssp') > -1) {
    window.location.href = jumpUrl;
    hideLoading();
  } else {
    $.ajax('/userlogin!hasLoginAjax.action').done(function (result) {
      var login = result.data.login;
      tooglePanel(login);
      if (login) {
        // 触发一次之后，解绑避免跳转导致再次触发
        $('#uc-jump-iframe').off('load');
        $.ajax('/client/account/overview/initAccount.action').done(function (_ref2) {
          var customerInfo = _ref2.data.customerInfo;

          if (customerInfo) {
            $('#panel-user-name').text(customerInfo.customerName);
            // 是否可以进入百青藤 1为进入百青藤
            window.experince = customerInfo.experince === 1 ? true : false;
            hideLoading();
          }
        }).fail(hideLoading);
      }
...
function tooglePanel(login) {
  $('#login-panel')[login ? 'hide' : 'show']();
  $('#user-panel')[login ? 'show' : 'hide']();
}
...
//# sourceMappingURL=index.59efd23.js.map